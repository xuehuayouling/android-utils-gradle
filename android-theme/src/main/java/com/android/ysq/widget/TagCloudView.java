...
public class TagCloudView extends ViewGroup {
	...
	private static final int DEFAULT_TEXT_BACKGROUND = R.drawable.y_tag_background;
	...
	public TagCloudView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.TagCloudView, defStyleAttr,
				defStyleAttr);
		...
		mTagBorderHor = a.getDimensionPixelSize(R.styleable.TagCloudView_tcvItemBorderHorizontal,
				DEFAULT_TEXT_BORDER_HORIZONTAL);
		mTagBorderVer = a.getDimensionPixelSize(R.styleable.TagCloudView_tcvItemBorderVertical,
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		/**
		 * 高度根据设置改变 如果为 MATCH_PARENT 则充满父窗体，否则根据内容自定义高度
		 */
	...
	private void initSingleLineView(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (mShowEndText) {
			...
			endText.setText(
					endTextString == null || endTextString.equals("") ? DEFAULT_END_TEXT_STRING : endTextString);
	...
	private int getSingleTotalHeight(int totalWidth, int totalHeight) {
		...
		for (int i = 0; i < getChildCount(); i++) {
			...
			if (child.getTag() != null && ((Integer) (child.getTag())) == TYPE_TEXT_NORMAL) {
				if (totalWidth + mTagBorderHor + mViewBorder + mViewBorder + endTextWidth + imageWidth < sizeWidth) {
					child.layout(totalWidth - childWidth + mTagBorderVer, totalHeight - childHeight,
							totalWidth + mTagBorderVer, totalHeight);
				} else {
		...
		if (endText != null) {
			endText.layout(totalWidth + mViewBorder + mTagBorderVer, totalHeight - endTextHeight,
					totalWidth + mViewBorder + mTagBorderVer + endTextWidth, totalHeight);
		}

		totalHeight += mViewBorder;

		if (imageView != null) {
			imageView.layout(sizeWidth - imageWidth - mViewBorder, (totalHeight - imageHeight) / 2,
					sizeWidth - mViewBorder, (totalHeight - imageHeight) / 2 + imageHeight);
		}
	...
	private int getMultiTotalHeight(int totalWidth, int totalHeight) {
		...
		for (int i = 0; i < getChildCount(); i++) {
			...
			if (totalWidth + mTagBorderHor + mViewBorder > sizeWidth) {
				...
				child.layout(totalWidth + mTagBorderHor, totalHeight - childHeight,
						totalWidth + childWidth + mTagBorderHor, totalHeight);
				totalWidth += childWidth;
			} else {
				child.layout(totalWidth - childWidth + mTagBorderHor, totalHeight - childHeight,
						totalWidth + mTagBorderHor, totalHeight);
			}
		}
			...
			for (int i = 0; i < tags.size(); i++) {
				...
				tagView.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						TextView tagView = (TextView) TagCloudView.this.getChildAt(finalI);
						if (tagView.getTag() != null && ((Integer) (tagView.getTag())) == TYPE_TEXT_NORMAL) {
